...
AFRAME.registerComponent('spe-particles', {
  ...
  update: function (oldData) {
    ...
    if (
      Object.keys(delta).some((x) => !['enabled', 'enableInEditor'].includes(x))
    ) {
  ...
  tickParticleSystem: function (dt) {
    if (this.data.relative === 'world') {
      let newPosition = toVector3(this.data.positionOffset).applyMatrix4(
  ...
  addParticleSystem: function () {
    ...
    let groupOptions = {
      texture: {
        ...
        frameCount:
          data.textureFrameCount >= 0 ? data.textureFrameCount : undefined,
    ...
    let emitterOptions = {
      type: SPE.distributions[
        data.distribution in SPE.distributions ? data.distribution : 'BOX'
      ],
...
/**
 * A-frame 1.3.0用にアップデート
 * https://github.com/harlyq/aframe-spe-particles-component/pull/7/commits/298ecf957ffaf441457b0965f94cf1ab01ae4017
 */
...
SPE.Group.prototype._triggerSingleEmitter = function (pos) {
  ...
  }, Math.max(emitter.duration, emitter.maxAge.value + emitter.maxAge.spread) *
    1000);
